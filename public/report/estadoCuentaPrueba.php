...
function RoundedRect($x, $y, $w, $h, $r, $style = '')
	...
	function Header(){
		...
		$this->SetXY($lw+40,$lh);
		$this->MultiCell(50,8,'Puente Piedra - Lima - Peru',1,'C');
		$this->SetXY($lw+90,$lh);
		$this->MultiCell(50,8,'www.munipuentepiedra.gob.pe',1,'C');
		$this->SetXY($lw+140,$lh);
		$this->MultiCell(60,8,'Telefono 219 6200 - 219 6201',1,'C');
		...
		$this->MultiCell(80,5,$direccionContrib,0,'J');
		...
		$this->SetXY($lw+$ls+35,$lh+5);
		$this->Cell($colSubCab[0][1],5,"",1,1,'L',true);
		$this->SetXY($lw+$ls+35,$lh+6);
		$this->MultiCell($colSubCab[0][1],3,'Est',0,'C');
	...
	function Footer(){
		...
		$this->SetFont('Arial','',10);
		//Número de página
		$this->Cell(79,5,'Municipalidad Distrital de Puente Piedra',0,0,'L',0);
		$this->SetFont('Arial','',8);
		$this->SetXY($marini,-10);
		$this->Cell(0,5,utf8_decode('Pagina N° ').$this->PageNo().' de {nb}',0,0,'R');
		$this->SetXY($marini,-10);
		$this->Cell(79,5,'GIT - Gestor Integrado Tributario',0,0,'L',0);
	...
	function WriteResumen($x,$h, $corte,$topmar){
		...
		$this->SetXY($x+55,$h);
		$this->MultiCell(80,5,'servicioalciudadano@munipuentepiedra.gob.pe',0,'L');
		$this->SetXY($x+135,$h);
		...
		$this->MultiCell(80,5,'Visitanos en: www.munipuentepiedra.gob.pe',0,'L');
		$this->SetXY($x+85,$h+5);
...
$Rs_grabar->Poner_MSQL("select  case when public.listas_adicional_per(cidpers) is null
								...
								round(sum(case when COALESCE(nestad2, '0')='0' 
											...
											else tmp_nimpmor end + case when nestad3='I' 
											then tmp_nimpmor else 0 end),2) as nmora2,
								round(sum(case when COALESCE(nestad2, '0')='0' 
											then (nimpins * nfacrea * nfacmor * tmp_ndiaven)::decimal(18,2) 
											else tmp_nimpmor end + case when nestad3='I' 
											then tmp_nimpmor else 0 end),2) as recargo2,
								trunc(round(sum( nimpins * 1 * (case when nfacrea = 0 then 1 else nfacrea  end))+ 
											sum(case when COALESCE(nestad2, '0')='0' 
												then (nimpins * nfacrea * nfacmor * tmp_ndiaven)::decimal(18,2) 
													else tmp_nimpmor end + case when nestad3='I' 
													then tmp_nimpmor else 0 end),2)+ sum(ncosemi),2) as total3,
								round(sum(nimpins * 1 * (case when nfacrea = 0 then 1 else nfacrea  end)),2 ) + 	
									round(sum(case when COALESCE(nestad2, '0')='0' 
										then nimpins * nfacrea * nfacmor * tmp_ndiaven else tmp_nimpmor end + case when nestad3='I' 
										then tmp_nimpmor else 0 end),2),
								...
								from (select a.cidpers, b.crazsoc as vrazsoc, b.direccf as  vdirfis,
											...
											a.cperiod, case when e.vobserv='IP' then i.vdescri when e.vobserv in ('SC','LP','RC','LC','PJ') then j.vdescri else '' end as dperiod,
											...
											((coalesce(left(a.cidpred,10),'') || ' - '|| coalesce(pl_function.verusopred(a.cidpers,a.cidpred) ,'') || ' - '||g.vnombre 		|| ' ' || h.vnombre
														...
														|| case when length(trim(f.dnlotes)) > 0 then ' Lte. ' || trim(f.dnlotes) else '' end) :: character varying(400)) else coalesce(e.vdescri,'')  end as TipoDe,
											...
											, COALESCE(case when a.ctiping in('0000000278', '0000000273') and a.nestado<>'I' then
	...
	while($N < $numRows){
		...
		if($tipoIngreso <> $row['ctiping'] or $predioContrib <> $row['tipode']){
			...
			*/
				...
				$pdf->SetXY($lw+$ls+35,$lh+$lz);
				$pdf->MultiCell($colSubCab[0][1],4,substr($row['vestado'],0,1),0,'C');
			...
			*/
			...
			if($row2['cperanio']<>$anioPeriodo 
				...
				if($isAmnistia == true){
					...
					switch($row['ctiping']){
						...
						case '0000000278':	#Arbitrios
							if(trim($row['cperanio'])<='2009'){ #2008
								$sumDescImpSol += $sumSinAmnistiaImpSol*0.9;

							}elseif(trim($row['cperanio'])>'2009' and trim($row['cperanio'])<'2014'){ # 2008 - 20014
								$sumDescImpSol += $sumSinAmnistiaImpSol*0.5; # 0.5
							}elseif(trim($row['cperanio'])=='2014'){
						...
						break;
						case '0000000313':	#FRACIONAMIENTO TRIBUTARIO
							$sumDescImpSol += 0;
							$sumDescRejuste += 0;
							$sumDescMontoTotal += 0;
							$sumDescAbonado += 0;
				...
				if($isAmnistia == true){ #$isAmnistia == true
						
					$ls=-5;
					
					if($row['cperanio']<'2015'){	#2014
						$textoBenenificio = 'Amnistia - Ordenanza 251-2015/MDPP';	#Ordenanza 230-2013/MDPP
